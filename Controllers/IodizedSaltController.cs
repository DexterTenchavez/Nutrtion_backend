...
{
    [ApiController]
    [Route("api/ReportDataEntry/iodized-salt")]
    [Authorize(Roles = "admin,staff")]
    public class IodizedSaltController : ControllerBase
    {
        private readonly IIodizedSaltService _service;

        public IodizedSaltController(IIodizedSaltService service)
        {
            _service = service;
        }

        [HttpGet("check-duplicate")]
        public async Task<IActionResult> CheckDuplicate([FromQuery] string storeName, [FromQuery] string barangay, [FromQuery] int purok, [FromQuery] int? excludeId = null)
        {
            try
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] IodizedSaltEntryDto dto)
        {
            try
            {
        ...
        [HttpGet("{barangay}")]
        public async Task<IActionResult> GetByBarangay(string barangay, [FromQuery] int year = 0)
        {
            try
            {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] IodizedSaltEntryDto dto)
        {
            try
            {
