...
{
    [ApiController]
    [Route("api/ReportDataEntry/pregnant-women")]
    [Authorize(Roles = "admin,staff")]
    public class PregnantWomenController : ControllerBase
    {
        private readonly IPregnantWomenService _service;

        public PregnantWomenController(IPregnantWomenService service)
        {
            _service = service;
        }

        [HttpGet("check-duplicate")]
        public async Task<IActionResult> CheckDuplicate([FromQuery] string womanName, [FromQuery] string barangay, [FromQuery] int purok, [FromQuery] int? excludeId = null)
        {
            try
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] PregnantWomenEntryDto dto)
        {
            try
            {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] PregnantWomenEntryDto dto)
        {
            try
            {
